---
title: Artifacts - Poems, Texts and Social Posts
type: meta
...
This page gathers artifacts that are made of language: a scanned journal page, a typed poem, an Apple Notes draft, a screenshotted tweet, a text-message bubble. The medium is the point. Where the concept images show the mythology drawing on the walls, these show it writing in the margins.

A few of them are self-labeling, meaning the artifact spells out its own unlock code in plain sight: the poem titled after its own first line, the tweet whose last line is the key. That is not sloppiness. It is a small joke the Hall likes to play, where the clue and the prize turn out to be the same object.
...
Read the words first, the mythology second. A diary line is a diary line before it is a prophecy. The most useful readings on this page start from what the text actually says and only then ask what it connects to, rather than treating every scrap of handwriting as a coded transmission from the beyond.

The links under each entry are deliberately few. A poem can gesture at a dozen ideas; the guide only points you to the connections that actually help you understand what you are reading.

---

## The artifacts

### #15 · DEAD LETTER DIARIES · `DEAD LETTER DIARIES`
...
### #65 · BASICALLY & POTENTIALLY · `EVERY HUMAN BEING IS A STAR`
...
A deliberately childlike pencil sketch captioned *"I was Sleeping on Carrot Juice":* two grinning, spiky-topped carrots squeeze juice down into a puddle where a tiny stick figure floats, arrow reading *"Me, sleeping."* The code is the caption verbatim, so the artifact keys itself (the same trick a few other rooms play, where the artwork simply states its own code). Seen alone it is an odd dream image. Seen where it actually lives, it is something else: this scene sits on a larger sheet keyed by a line from *Soul Is A Star* — *"we fall down as seeds on the ground"* — and that one sheet supplies the codes to three further rooms, including this one. The page is not an illustration of unrelated doodles. It is a small source of connected rooms, and looking closely at one artifact changes where you are able to go next.

What the picture shows is a sleeper being fed. A carrot is a root, an organ that stores energy out of sight, and here the carrots nourish a figure captioned *"Me, sleeping"* on a page about seeds and dormancy. Stored energy formed below awareness, sustaining a self that has not woken yet — that reading fits the image and its parent, though it remains a reading. No rabbit is drawn here, so a lure would need another anchor, and the drawing does not record its maker's age or a particular childhood event. The absence of a carrot in the released lyrics settles nothing: the sheet's own key *is* a released lyric, and the humour survives the reading intact.
