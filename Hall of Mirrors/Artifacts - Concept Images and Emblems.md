---
title: Artifacts - Concept Images and Emblems
type: meta
cssclasses: ["type-meta"]
status: published
confidence: mixed
source_basis: ["Hall of Mirrors artifacts", "released music", "public posts", "official videos"]
---
# Concept Images & Emblems

Some Hall of Mirrors artifacts are beautiful because they look finished.

These are not those.

These are the stranger, more useful objects: diagrams, emblems, board fragments, code-images, hand-drawn maps, symbolic props, screenshots, scans, and little conceptual grenades that seem to have fallen out of the machinery while the mythology was being built.

This is where the Hall stops whispering and starts drawing on the walls.

## What this section collects

This page gathers artifacts whose image, title, code, or content points directly at a major concept in the Jagwar Twin world. These are not just pictures of things. They are pictures that behave like arguments.

A crown with a snake is not only a crown with a snake. A rabbit at a gate is not only a rabbit at a gate. A handwritten note that says *"the value of Nothing"* is not just a motivational poster for extremely tired mystics.

These artifacts function like visual glossary entries. They help define the project's recurring ideas: attention, the pattern, the empty room, the gate, the world, the body, the senses, the sphere, the star, the flood, Eden, the rose, the sword, the dove, the rabbit, the mirror, and the question of how a self becomes legible without becoming trapped.

## Why these artifacts matter

The songs tell the story emotionally. The characters personify it. The interviews give us language for it. The concept images show the system *thinking*.

Many of these pieces feel like working notes from the mythological engine room: sketches from Roy's *"I'm 33"* board, fragments of symbolic math, handwritten cosmology, diagrams of the person, old personal objects turned into relics, and images where a code doesn't merely unlock the room but explains what the room is trying to say.

They matter because they show the bridge between private pattern and public art. You can see an idea before it becomes a song, before it becomes a character, before it becomes a room, before it becomes something the rest of us start arguing about at 1:00 a.m. like very normal people.

## How to read them

Start with the object itself. What is actually visible? Then the title. Then the concept links.

The order matters. If you begin with the wildest possible interpretation, everything becomes a prophecy and nothing becomes useful. The better question is not *"How mystical can I make this?"* but *"What does this artifact connect, and how strongly?"*

A strong reading usually has several anchors: the image itself, the title, a known Roy statement, a song tie, a repeated symbol, or a matching diagram elsewhere in the Hall. A weak reading has vibes and enthusiasm. Vibes are welcome here, but they are not allowed to drive without a licensed adult in the car.

## The main kinds of concept artifact

**Board fragments.** A large number of these appear to come from Roy's *"I'm 33"* board-world: handwritten math, music diagrams, Tree of Life structures, Happy Birthday contours, Adam/Eve calculations, Platonic ratios, Saturn squares, sensory hierarchies, and other attempts to map reality as pattern. They are not polished album art. They are closer to field notes from the inside of the inside.

**Emblems.** Some artifacts work like icons: the dove, the crown, the sword, the rose cross, the owl, the rabbit, the cube, the gate, the sphere. They condense a concept into one object. When an emblem repeats across songs, tattoos, drawings, videos, and rooms, it becomes part of the visual grammar.

**Personal objects turned symbolic.** Not everything here is a diagram. Some pieces are ordinary objects: a dog, an old CD, a desert house, a childhood image, a cow, a beach photo, a venue clip. The Hall treats these as relics, not because ordinary life is being inflated into mythology, but because the mythology keeps returning to ordinary life. The cosmic map only matters if it eventually touches a body, a family, a place, a memory, a wound, a joke, a dog named Potato Chip. That is not a side note. That is the point.

## Major threads on this page

**Attention becomes a god.** The *Marduk* artifact turns attention into an ancient figure: many-eyed, mythic, enormous, and slightly terrifying. The attention economy is not just a platform mechanic here. It becomes a deity demanding sacrifice, and the song world answers with [[Pay Attention (feat. little luna)]].

**Nothing becomes valuable.** Several artifacts circle emptiness: the empty room, the empty mirror, zero, *the value of Nothing*, absence, invisibility, the strange usefulness of becoming no one. This is not simple nihilism. Emptiness can mean loss, erasure, spiritual clearing, or the open space where something truer can finally appear. The hard part is telling the difference.

**The self becomes a map.** The *Enlightened Being*, *Sense Hierarchy*, body diagrams, and Tree-like sketches treat the human being as layered architecture: body, senses, feeling, thought, will, love, self, universal self. The self is not flat. It has levels, rooms, gates, thresholds, inner instruments, and possibly a very concerning amount of paperwork.

**The world has gates.** The rabbit at the Gate of the World, the *Gated* video, the *Eden Cube*, and related images all return to threshold imagery. A gate is a test of position. Are you outside looking in? Inside trying to leave? At the edge of the world? The Hall loves gates because the whole project is about crossing from performance into recognition, from image into self, from pattern into life.

**The truth has shape.** *"The truth is a sphere"* is one of the cleanest concept statements in the archive: truth is not a flat slogan but a whole object, seen differently depending on where you stand. That idea quietly governs the guide itself. A song, a post, a drawing, and a video can all be different angles on the same thing. None of them alone is the whole sphere. Together, they become harder to dismiss.

**The old self becomes an artifact.** The Eye Alaska CD, old footage, childhood images, and personal-history objects show the Hall preserving earlier versions of the person. These are not merely nostalgic. They are archaeological: the kid, the band, the abandoned path, the old name, the earlier sound, the thing that did not disappear just because a new mythology arrived.

## A note on the "I'm 33" board

Many of these concept images appear to be fragments of the larger thought-system visible in [[Hall of Mirrors/The I'm 33 Board]]. That board shows Roy working through music, number, geometry, theology, identity, and pattern as one connected field. Several artifacts here seem to be individual pieces of that board-world minted into the Hall: not separate ideas, but pieces of the same cosmology broken into collectible rooms. The board helps decode the artifacts; the artifacts show the board learning how to become mythology.

## What not to do

Do not treat every image as equally central. Do not treat every code as proof of a grand hidden plot. Do not collapse personal objects into symbols so aggressively that the human disappears. Do not make the map so clever that it stops being true.

The best artifact readings leave the object more visible than they found it. If an interpretation makes the artifact feel less human, less funny, less strange, or less specific, it is probably trying too hard.

## How this page is organized

Each entry below carries the artifact's number, title, unlock code, image or video, a short reading, and links to the strongest related songs or concepts. The links are intentionally selective. A concept image can touch twenty things in theory; the guide only links the connections that help you understand what you are looking at. The goal is not to win the maze. The goal is to notice how it was built.

---

## The artifacts

### #5 · HELLO, ATTENTION (µå®∂¨˚) · `MARDUK`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xdfdd1b81a97b04e4b0b7459d0dd40810e63e090957a2e57219ac23ce0f28d919..mp4" poster="https://plumcreekranch.com/jagwar/artifacts/4.png" width="320" controls preload="none"></video>

A segment of Roy's *"I'm 33 this year"* board: a pasted Mesopotamian winged-deity relief (Layard's plate of the Neo-Assyrian Ninurta relief, historically misidentified as Marduk battling Tiamat) with a green-highlighted cell beneath it reading *"eyes all around head"* — Marduk's *Enuma Elish* attribute, the four all-seeing eyes. The board elsewhere poses the exact worksheet question this answers: *"How to anthropomorphize attention? → Marduk."* The title itself is the answer typed in the Mac Option layer (µå®∂¨˚ = "marduk"), and Roy confirmed the whole equation outright: *"Marduk. You've got to pay Attention."* This is the moment the attention economy gets cast as an ancient Babylonian god, the deity-of-Attention the released song then dramatizes.
**Connects to →** [[Pay Attention (feat. little luna)]] · [[Hall of Mirrors]] · [[Concepts/The Pattern]] · [[Sir Lucius]] · [[Roy]]

### #9 · I'M NOT HERE · `I'M NOT HERE`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x1859535ea2a83a646006b26c8fb24b890e1d8c173843f18f9d99ed82a55435e5..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x1859535ea2a83a646006b26c8fb24b890e1d8c173843f18f9d99ed82a55435e5..jpg" width="320" controls preload="none"></video>

Thirteen seconds of phone video of Roy at night in Florence's Piazza del Duomo, the striped marble of the cathedral and Baptistery lit behind him, wearing novelty sunglasses topped with a tiny palm tree and flamingo, grinning beside a laughing companion; a glowing Christmas tree places it in the holiday season, and the clip ends with him turning and walking away into the piazza. The title cuts against the festive image: a man mugging for the camera under a cathedral while declaring his own absence — biographically, the years he was off the public map; in Hall of Mirrors terms, the maze's recurring play with presence, absence, and the unwitnessed self. The closing shot, his back receding into the piazza, literally performs the title. So does the audio: off-camera a man and a woman start screaming his name into the peaceful square, *"Roooy, where are you,"* and the clip ends with the call still unanswered, a room called I'M NOT HERE built around people asking exactly where he is. And there is a quieter layer in the setting. The Baptistery's gilded east doors are Lorenzo Ghiberti's, the panels Michelangelo famously called the *Gates of Paradise*; whether or not the camera meant it, the man declaring himself not-here stands in the dark outside a door named Paradise, called by name, already walking away.
**Connects to →** [[Move To You]] · [[MAGIK]] · [[Hall of Mirrors]] · [[Roy]]

### #16 · THE EMPTY ROOM · `RAINBOW BRIDGE`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xa53f8677e36ba5ad03dbefe1ee2baa69151a39fa947f876b0e4af598a7c56397..png"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xa53f8677e36ba5ad03dbefe1ee2baa69151a39fa947f876b0e4af598a7c56397..png" width="300"></a>

A hand-drawn sketch, and the maze's transformation formula drawn as a diagram. What is actually on the page: a rainbow arch with no color in it at all, concentric graphite bands only, with a small empty doorway at its base; two open-topped pillars lettered **J** (left, dark-hatched) and **B** (right, light), the Jachin and Boaz of Solomon's Temple; **three serpents soaring in the air above the arch**; the caption *"The Empty Room"*; and below it a **two-faced, monkey-like creature**, its head split by a zigzag, the two faces turned opposite ways with arrows pulling in both directions, one body trying to move both ways at once. Those figures are not generic esoterica. In the whole maze, monkey, serpent, and flight come together in exactly one other room: [[Artifacts - Gallery 4 (113-149)#125 · THE SERPENT MUST TAKE FLIGHT|room 125]], titled *"THE SERPENT MUST TAKE FLIGHT"* and unlocked by *"THE MONKEY MUST BECOME THE BIRD."* This drawing reads as that formula's picture: the divided monkey below is the one that must become the bird, and the serpents overhead have already taken flight. In the Theosophical stream Roy's board draws on, the *"rainbow bridge"* (antahkarana) is the span a person builds across their own divided nature, which compresses the whole sketch into one statement: **the divided monkey must cross the empty bridge to take flight.**

The emptiness is the other half of the design. The composition closely matches the Fourth Apocalyptic Seal Rudolf Steiner published, a rainbow spanning two pillars lettered J and B in Steiner's rare J-left/B-right order (the reverse of the Masonic tracing-board and the Tarot High Priestess), with the seal's entire subject stripped out: the radiant figure, the open book, the cloud, the sea and land, and the color itself. Steiner is a named influence, and the match is held as a strong read, not a statement of intent. What remains is the room of the title, an image with no subject, so whoever takes the room becomes its subject; this guide's author reads the invitation as *"see what I, the consumed, see from the seat I left empty for you."* The empty doorway is the crossing not yet made, the same double-edged *empty* held at [[Concepts/The Empty Mirror]], and as an early artifact the sketch plants the pillars-and-middle schema the Kabbalah cluster later formalizes: Roy on one pillar, Lucius on the other, the human walker as the in-between.
**Connects to →** [[It's Your Time]] · [[Great Time to Be Human]] · [[Kabbalah - The Tree of Life]] · [[Concepts/The Empty Mirror]] · [[Concepts/Duality]] · [[Concepts/The Divide]] · [[Roy]]

### #19 · ARTIFACT 187 · `DEATH OF MY GENERATION`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x84e81fedd884f04c7b3a46eb5b3542ca5d9d3443076d1361a93745ee829912f6..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x84e81fedd884f04c7b3a46eb5b3542ca5d9d3443076d1361a93745ee829912f6..jpg" width="300"></a>

A teal-washed four-panel collage of *Subject to Flooding*-era (2018–19) imagery: Roy in a blue beanie on a scarred desert highway; a backlit grid of cattle skulls; the debut cover shot of Roy submerged in turquoise water, face buried in his hands, torso marked with handwritten words; and a second pool photo with *"FEAR"* inked across his collarbone and *"ALIVE"* on an arm. The title *"ARTIFACT 187"* borrows the California penal code for murder, so the artifact numbered with the murder code opens with *"DEATH OF MY GENERATION,"* the *Happy Face* bridge lyric naming the casualty. The skull wall literalizes the death; the drowning-baptism cover and the FEAR/ALIVE body-writing stage the suppressed-self material *Happy Face* would later diagnose. The phrase carries campaign weight too: *deathofmygeneration.fun* was the Dec 2020 *Happy Face* microsite (the *"Are you content?"* smile-gate) that predates the Hall of Mirrors, so this folds the debut-era visual self into the lyric that killed it off.
**Connects to →** [[Happy Face]] · [[Roy]] · [[The Mask]] · [[Concepts/The Circus]]

### #30 · KHAWAH · `26`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x9cce9eb03c9f29c6481fca9f0f942b15bef0bbbc47fda0ddb44df157019835d9..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x9cce9eb03c9f29c6481fca9f0f942b15bef0bbbc47fda0ddb44df157019835d9..jpg" width="300"></a>

This is the parent source sheet for the direct crops called *Forbidden City*, *Fibonacci Rabbits*, and *Logos (Ratio)*. Those rooms keep their own identities and codes; their repeated marks do not become independent witnesses.

A dense handwritten page of Roy's esoteric mathematics, effectively a leaf of the *"I'm 33"* board corpus. The code **26** is written on the page three times: the gematria of YHWH (10+5+6+5=26), *"ONE LOVE = YHWH = 26,"* and *"ADAM − KHAWAH = 26"* — which supplies the title. Khawah (Chavah/Havvah) is Eve; in Hebrew gematria Adam (45) minus Chavah (19) leaves 26, so the difference between the primal pair is literally the name of God: love as the remainder that binds the split-in-two back to One. Around that core the page asks *"what is music?"* and answers with Happy Birthday melody contours (*"the greatest Song ever written," "When was I Born?"*), then stacks the tetractys (*"the essence of man"*), a magic square, a later completed Lambda derived from Plato's *Timaeus* World-Soul construction, a Fibonacci rabbit tree, and the Scott Olsen phrase *"buddhas, shamans, and microtubules."* Read slowly, the sheet invites a synthesis: number, music, love, and God held in one computation. That synthesis is a reading, not a direct statement of Roy's intent. Plato supplies the seven boundary quantities and harmonic construction, not this finished ten-node diagram; a full Lambdoma is a different ratio matrix. In the completed figure, every horizontal row (2 · 3, then 4 · 6 · 9, then 8 · 12 · 18 · 27) steps by the same 3:2 ratio, the perfect fifth, so the page's number-figure answers *"what is music?"* through relation. The magic square also checks out: every row, column, and diagonal sums to 66, with the room's own code 26 inside the grid. And the page moonlights as a concordance for its wing of the maze: *"the essence of man"* is room 50's unlock code, *"Logos (Ratio)"* is room 146's name and the Elephant-Tortoise-Eagle-Atlas caption beside it is that room's code, the Fibonacci rabbits are room 120's subject, and the Lambda here is labelled *"Forbidden City,"* room 93's name. One sheet carries at least four other rooms' identifiers: one room's page holding the names and codes of other doors.
**Connects to →** [[Another Way II Heaven]] · [[SOL]] · [[Concepts/The Pattern]] · [[Kabbalah - The Tree of Life]] · [[Roy]]

### #31 · IF THE SEA HAS ENOUGH WILL · `THE PATTERN`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xeaea42961328b71c465c11c237bae8498c135938350565b81d0cb9020a8fa3d3..jpeg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xeaea42961328b71c465c11c237bae8498c135938350565b81d0cb9020a8fa3d3..jpeg" width="300"></a>

A family-archive snapshot: a toddler (almost certainly young Roy, early-1990s film era) in a comically oversized red felt hat and checkered sweater sits on a boat in a Southern California harbor, staring gravely at the water while an adult in Wayfarers (plausibly his father) sits behind him. The title reads the scene as cosmology, granting the sea agency (*"enough will"*), the ocean as the boundless field the small self floats on. And the room's own code, *THE PATTERN*, points to the hub of Roy's board cosmology (the *"I'm 33"* Great Design / One-become-Two cosmogony), framing the child at the rail as an early node in the design he later maps. The debut is, after all, named *Subject to Flooding* — this child is already subject to the water shown here.
**Connects to →** [[Shine]] · [[Concepts/The Pattern]] · [[Subject to Flooding]] · [[Roy]]

### #32 · O · `THE VALUE OF NOTHING`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x538115b37f8d235538a6c63b12248a39c79a813ad927a7d2a7e7595457de89a9..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x538115b37f8d235538a6c63b12248a39c79a813ad927a7d2a7e7595457de89a9..jpg" width="300"></a>

A single line of what reads as Roy's handwriting in black ink: *"the value of Nothing"* (only the N capitalized), so the artifact literally displays its own code in what reads as the artist's hand. The title *"O"* makes the phrase visual: O is the zero, the empty circle, the shape of nothing. The phrase is grounded biography, not just aphorism — it appears verbatim on the *"I'm 33"* board alongside *"the inside of the inside"* and *"nothing changed and everything changed,"* and it ties to The Empty Mirror, Roy's tattoo he describes as *"looking at yourself and seeing nothing."* The wording also inverts Oscar Wilde's cynic who *"knows the price of everything and the value of nothing":* in Roy's cosmology nothing is not worthless, it is the thing whose value you finally learn, the dark empty center out of which the whole Hall of Mirrors reflects.
**Connects to →** [[Great Time to Be Human]] · [[Concepts/The Empty Mirror]] · [[The Circle]] · [[Roy]]

### #35 · YELLOW & ELEPHANT 2 · `CHAIN REACTION`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xc7f87ec37122f93afd711dda88c305c17f91b8edbc366215c8f6f8f8bca2faf0..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xc7f87ec37122f93afd711dda88c305c17f91b8edbc366215c8f6f8f8bca2faf0..jpg" width="320" controls preload="none"></video>

A short photogrammetry turntable scan of a still-shrink-wrapped retail CD of *"Yellow & Elephant,"* the 2008 Fearless Records EP by **Eye Alaska**, Roy English's Orange County band before Jagwar Twin. The yellow cover with its many-trunked black elephant, the EYE ALASKA spine text, a Fearless $5.99 sticker, and the barcode are all legible as it rotates on weathered crate slats. The code *"CHAIN REACTION"* is biographical, not esoteric: Chain Reaction is the Anaheim all-ages venue Eye Alaska named as *"our home venue"* in a 2010 interview. Scanning a mass-produced, still-sealed store copy (price tag intact) turns commercial ephemera from Roy's youth into a preserved archaeological object — the Hall's museum-of-past-selves, identity as layered relics.
**Connects to →** [[Hall of Mirrors]] · [[Roy]]

### #37 · I AM CHRITMAS · `I AM CHRISTMAS`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x1253b97f8166ce69a25958b7cac2eaf706deb8f0bf520ff9fa6a2b975f8d0cf3..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x1253b97f8166ce69a25958b7cac2eaf706deb8f0bf520ff9fa6a2b975f8d0cf3..jpg" width="300"></a>

A two-panel sketch: on the left, an ankh-like figure (a looped head atop a vertical stem crossing a horizontal bar) with its three limbs numbered 2, 3, 1; on the right a plain cross with *"ROSE"* across its beam; an arrow shows the first becoming the Rose Cross. This is the same hand and cluster as the board's handwritten *"ROSE (cross),"* which sits beside *"I.H.S."*, *"the Pearl of Great Price,"* and *"VI — Love — Sacrifice,"* a compact Rosicrucian/Christian-mystical knot. The code (Roy on the day: *"I am Christmas! omg yes!"*) reads the drawing esoterically: Christmas is the Christ-mass, the birth of Christ, and the Rose Cross is the rose (soul, love) blooming on the cross of matter — so the body-figure turning into the Rose Cross declares that the Christ-birth happens in the self. It sits naturally in *33*, an album named for the age at which, in Roy's numerology, Christ-consciousness is attained.
**Connects to →** [[Roy]] · [[Mary]] · [[The Rose]]

### #40 · POTATO CHIP · `POTATO CHIP`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x20b823729b7a5f6c459b22d543b7a74b1cd49e1da51ca1ec1b2a1ff839d461cd..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x20b823729b7a5f6c459b22d543b7a74b1cd49e1da51ca1ec1b2a1ff839d461cd..jpg" width="300"></a>

A sideways, unedited phone snapshot of a yellow Lab-type dog on grey carpet in a home hallway, flower-shaped collar tag, a reddish patch at the chin, looking straight up at the camera; a duffel, sage walls, and a framed picture place it in ordinary domestic life. The code is just the dog's name: Roy's own reply, *"My dog tried that once and his whole face swelled up. Potato Chip never ate bees again,"* confirms Potato Chip is his dog. Within the cosmology, rooms like this are the counterweight to the sigils and Kabbalah diagrams: raw creaturely life from the biographical Roy, presented un-precious (not even rotated upright), the unglamorous heart the myth machinery is built around.
**Connects to →** [[Roy]]

### #41 · PERSPECTIVE · `JOSHUA TREE`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x5648ac4aedb6afc8efe58a5a5cd99f2669b286cb3a2d00b784b072bcbb5f3ce6..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x5648ac4aedb6afc8efe58a5a5cd99f2669b286cb3a2d00b784b072bcbb5f3ce6..jpg" width="320" controls preload="none"></video>

A 33-second cinematic vignette of a desert homestead with a white geodesic-dome house: timelapse clouds, a golden-hour tracking shot following three people toward the property (two wearing shirts printed *IM NOT HERE*, Roy's tattoo text without its apostrophe), a dusk wide of the road, and a final hold on the dome under a dense star field before fading to black. The code simply names the place; the title *"PERSPECTIVE"* is enacted formally — one location shot from shifting vantage points and times of day, the same thing seen differently. The *IM NOT HERE* shirts loop it back into the presence/absence joke.
**Connects to →** [[Move To You]] · [[Roy]]

### #44 · THE ANCIENT CHILD · `JUST WOKE UP`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xe1ced4d6a28de20b22b20717d8d5e65fa69cf421756a765aa2e34053bbf31d3a..png"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xe1ced4d6a28de20b22b20717d8d5e65fa69cf421756a765aa2e34053bbf31d3a..png" width="300"></a>

A pencil sketch that makes its title literal: an ancient, heavily wrinkled face with weary, heavy-lidded eyes set on a child's-drawing body, stick arms flung out and a ghost-sheet torso with a zigzag hem, so the old man and the child occupy one figure drawn with a child's hand. The code *"JUST WOKE UP"* casts the figure as something ancient that has only just awakened, squarely on the awakening axis Roy publicly sources to [[Gurdjieff]] (*"man is asleep… he must wake up"*). It sits beside the catalog's Lewis Carroll old-man cluster, where extreme age keeps being viewed through a child's eyes — the senex and the puer as one.
**Connects to →** [[Good Day]] · [[The Watchers]] · [[Roy]]

### #45 · BLINKING (2/2) · `C876`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x211f837e4e8c6dd9e52ff46681309dc5a967ebadfe76999837fa08c9d2e3c4bd..png"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x211f837e4e8c6dd9e52ff46681309dc5a967ebadfe76999837fa08c9d2e3c4bd..png" width="300"></a>

The second of two screenshots of an Apple Notes poem titled *"Blinking 🌞"* (Nov 21, 2021, in Roy's "Songs" folder). This half runs a cosmological regress — Moon to Sun to *"the Fiddle with Strings that plays every song,"* behind which stand a Player, then a Symphony Master, then *"a mystery"* — before collapsing the chain inward with the punchline: *"Where do thoughts come from? Up on a hill / Of course! It's Golgotha / The skull that you think in / It's just you inside yourself / While you're blinking."* (Golgotha is Aramaic for *"place of the skull,"* so the cosmic hill of origination turns out to be your own head.) It echoes Roy’s tweet from two weeks earlier, *"We're almost literally in the eyeball of God as he/she/they are blinking."* The who-is-behind-the-song hierarchy resolves, characteristically, inward rather than upward. Read against the maze's own key-ring, the regress argues two codes in verse: each mover standing behind the last is #121's key, *"THERE ARE THINGS BEHIND THINGS,"* and the inward landing is #10's, *"INSIDE OF THE INSIDE,"* which is also The Circle's central refrain. One more wink: a poem whose last words are *"While you're blinking"* hangs in a room named BLINKING, the artifact carrying its own room's name in its closing line.

As a **Songs-folder** object, its larger function is to keep asking what stands behind a performance until the apparent master becomes another intermediary. That is the return-to-source motion [[I Like to Party]] interrupts and *[[The Interview]]* later stages as Sir Lucius replacing soul and vulnerability with streams, hits, and the algorithm. The cross-work join is interpretive; none of the three works claims direct dependence on another.
**Connects to →** [[The Circle]] · [[Concepts/Source vs Reflection]] · [[Concepts/The Voice]] · [[I Like to Party]] · [[The Interview]] · [[The Thread]] · [[Roy]]

### #46 · FLOW FORTH AS A THIRD · `W876`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x22d49cdf9a54d186cb3d82bbd8aefa5cd098a3b0369a5988ba67da110dd7c563..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x22d49cdf9a54d186cb3d82bbd8aefa5cd098a3b0369a5988ba67da110dd7c563..jpg" width="300"></a>

A candid photograph looking up at a venue balcony: an older blonde woman in a floral kimono-style jacket smiles warmly down toward an unseen stage, standing behind black safety netting beside younger companions, a red gothic stained-glass window glowing behind her, a broadcast camera on the wall implying a filmed Jagwar Twin show. Her identity is undocumented, but the intimate framing reads as a loved one, plausibly Roy's mother, watching him play. The title invites a Kabbalistic reading: in the Tree of Life the third emanation is Binah, the Great Mother, the seat Roy elsewhere renders as *"Queen Binah / GOD the Mother on Saturn,"* so *"flow forth as a third"* casts the watching woman as the maternal third from whom life emanates. A community variant transcription, *"Flow Forth as a Child,"* offers the mirror reading: the child she watches on stage.
**Connects to →** [[Roy]] · [[Mary]] · [[Kabbalah - The Tree of Life]]

### #50 · BUDDHAS, SHAMANS, AND MICROTUBULES · `THE ESSENCE OF MAN`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xbdea3bcd3973fd1dffdce0259fc9d32ae9d279aca33ed8cbdabd4f43f618a03e..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xbdea3bcd3973fd1dffdce0259fc9d32ae9d279aca33ed8cbdabd4f43f618a03e..jpg" width="300"></a>

A handwritten pencil page headed *"Computations of GOD,"* with a tetractys-like triangle of dots labeled *"The essence of man"* (the code, written on the page), a column of small rings reading as a microtubule cross-section or a string of pearls, and notes: *"Theory of Existence / the existence of the Soul?", "buddhas, shamans, and microtubules", "5:3 Ark",* and *"the Pearl of Great Price."* The title is lifted from Scott Olsen's 2007 *Quantum Mind* abstract, *"Resonance and Consciousness: buddhas, shamans and microtubules,"* which builds on the Penrose–Hameroff idea that consciousness arises from quantum computation in neuronal microtubules, with meditative and shamanic states as deeper access to that substrate — Roy asking, in one breath, whether the soul is literally a *"computation of God"* running in the body. *"5:3 Ark"* matches the Ark of the Covenant's proportion (a golden-ratio approximation), folding scripture into the same math-of-the-sacred register. This is definitional work-in-progress for the Upload cosmology: before you can upload a consciousness, you have to define what one *is*.
**Connects to →** [[Soul Is A Star]] · [[Concepts/The Pattern]] · [[Concepts/The Upload]] · [[Roy]]

### #53 · GREY IN A# · `PITUITARY GLAND OF AN OWL`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xc9821059859ee4afcbb77119fcc02798d90db2b4d6486ad6339a34f5287deb07..jpeg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xc9821059859ee4afcbb77119fcc02798d90db2b4d6486ad6339a34f5287deb07..jpeg" width="300"></a>

A taxidermied barn owl in a glass museum diorama: wings spread mid-flight, a *"BARN OWL"* placard, dried grasses, window reflections on the case — a specimen of night-sight frozen dead behind glass. The code *"PITUITARY GLAND OF AN OWL"* sounds like a witch's-brew ingredient but lands on documented territory: [[Alice Bailey]] makes the pituitary the esoteric *"cave in the head"* of the Leo labour, so the code names the organ of inner sight lodged inside the bird of night vision and wisdom (Athena's owl). The coordinate arithmetic holds here too: the owl is rung 2 of the board's animal ladder, the seat the Tree gives to Chokmah, Wisdom itself, and the pituitary sits in the head at station 9 of the body count, 2 and 9 making eleven. The image then ironizes it: the owl's *"gland"* is preserved but inert, wisdom taxidermied rather than alive, grey (the title's liminal non-color) rather than light or dark.
**Connects to →** [[Concepts/The Labors of Hercules]] · [[Roy]]

### #55 · CAUTION, ALL KINGS END · `CAKE`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x29c72dbd4ba375457faa384e87e584f5362540b5b955121f4c43e2513739729d..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x29c72dbd4ba375457faa384e87e584f5362540b5b955121f4c43e2513739729d..jpg" width="300"></a>

A black-and-white studio photograph against a void: a sword driven point-down through a jeweled royal crown with ermine trim, a live ball python coiling around the blade and threading through the crown's empty arches — a staged vanitas of power with no head to wear it. The title is an acrostic, *Caution All Kings End = CAKE*, which Roy confirmed, and the pun does real work: the gravest possible sentence (all sovereignty dies) collapses into party confection, the same bait-and-switch as *"bake a truth with a lie / then we sell 'em the cake,"* with a *"let them eat cake"* echo of regimes that fall. The serpent winding through the hollow crown reads as cyclical time: kings come and go while the shedding, continuous snake outlasts each one — exactly The Circle's cosmology of successive kings inside one repeating story. The Hall's bluntest memento mori: the crown is empty, the sword has already fallen, and the code you need is dessert.
**Connects to →** [[The Circle]] · [[I Like to Party]] · [[Roy]]

### #56 · ROY STRINGS · `SPEED OF SOUND`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x13c8a455aa0a4fe21bc7db273b9f395e8888775c64a25696e1d590439f5b6800..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x13c8a455aa0a4fe21bc7db273b9f395e8888775c64a25696e1d590439f5b6800..jpg" width="300"></a>

A scan of Roy's handwritten acoustics notes: two physics formulas, one for the fundamental frequency of an open pipe (*"Speed of Sound / 2× length of cylinder"*), one Mersenne's law for a tensioned string. The code *SPEED OF SOUND* is literally the underlined numerator of the first equation, so the room's key is written on its own face. The scrap sits inside Roy's frequency cosmology — the board's Kybalion *Vibration* cell, the pinned Tesla *"energy, frequency and vibration"* quote, his 2020 statement that *"everything is a vibrational frequency."* The title reads the equations biographically: Roy studying the actual physics beneath his stated practice of tuning music as vibration-medicine, the tensioned string as the instrument (or self) whose tension, mass, and length set its fundamental tone.
**Connects to →** [[Concepts/The Sacred Truths/Hermeticism - As Above So Below]] · [[Concepts/The Pattern]] · [[Roy]]

### #57 · VENUS · `DLD`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x0c6a332117c7270b3be98acbcbbcf3161369e9ab06527609378bf71865507acc..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x0c6a332117c7270b3be98acbcbbcf3161369e9ab06527609378bf71865507acc..jpg" width="320" controls preload="none"></video>

A short silent loop of a 3D scan of a real handmade object: weathered silver-gray wooden slats with flecks of yellow paint, framing a mounted wood panel bearing a black woodcut-style print of a dove beside a single crossing sprig, head bowed, rays of light around its head. The Venus-equals-dove equation is native to the gallery itself (another artifact is literally titled *VENUS IS A DOVE*), and the basic symbol map places Roy at Netzach/Venus, with the dove (named Cygnus), the sword and the heart, so this reads as Roy's own heart-emblem — the Venus/Netzach heart-pole of the Heart/Mind split, rendered as a folk icon. His Venus self-identification is independently documented (*"I'm Venusian, but it was great being on Mars for awhile"; "venus as a boy"*).
**Connects to →** [[Down to You]] · [[The Dove]] · [[Kabbalah - The Tree of Life]] · [[Concepts/Duality]] · [[Roy]]

### #58 · A STORM IN THE CHILD'S FOUNTAIN · `A STORM IN THE CHILD'S FOUNTAIN`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xd521f8508535492b47ed3604214cf011539be0ad5c1ae668f4dac8fc3a4d8069..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xd521f8508535492b47ed3604214cf011539be0ad5c1ae668f4dac8fc3a4d8069..jpg" width="320" controls preload="none"></video>

A short looping 3D scan of a black-and-white Holstein-patterned cow (small horns) rotating on a concrete disc against black. It is visibly the same scanned model as the *"Grandma VI's Cow"* artifact, so the family cow from Roy's childhood recurs here under a different title. And the different title is itself a relic: *A Storm in the Child's Fountain* is an Eye Alaska track title, a song from Roy's own late-2000s band, so the childhood keepsake now rotates under the name of a song written on his way out of that childhood. The code reads as the fountain-of-youth motif with a disturbance inside it: *"the child's fountain"* is childhood itself as a source, and the *"storm"* is the hidden grief or violence stored in it — Roy's documented habit of treating youth as a container concealing something explosive. Placing the pastoral family cow under that stormy title turns a tender rural memory into one of the Hall's memory-with-a-wound artifacts. The title also belongs to a register the guide had not assembled: **"fountain" occurs in exactly two Jagwar Twin lyrics.** *Great Time to Be Human* has the girl *"who drank from The Fountain,"* and *Good Time (1:08)* rules *"**Youth is not a fountain**, it's a hand grenade in hiding."* This artifact is the third variation on the same buried idiom — the fountain of youth — and all three run the same operation on it: the innocent vessel with the violence already inside. A girl drinks from it; a lyric denies it and swaps in a hidden grenade; and here the child's fountain simply contains the storm. The grenade half of that *Good Time* line is performed two shelves away, on [[Artifacts - Gallery 2 (38-74)|#76 Lemon the Rabbit]]'s acorn.
**Connects to →** [[Roy]] · [[Mary]]

### #61 · THE TRUTH IS A SPHERE · `ENLIGHTENED BEING`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x1b7d38676a6aaf1756b914985f67f995806a719e38f4e6f81a9b598b1eaf2b91..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x1b7d38676a6aaf1756b914985f67f995806a719e38f4e6f81a9b598b1eaf2b91..jpg" width="300"></a>

A hand-drawn concentric-sphere map of the person captioned *"THE Enlightened BEING":* ten nested rings run from *10. body* at the rim inward through unconscious, thinking, feelings, will, love, centre, and *1. Self,* down to *universal will / universal love / Universal Self* around a black center point, double-headed arrows spanning the radius. It is one encoding of the 10-stage scheme on Roy's board, which carries the same decad in Japanese stage-labels and Tree-of-Life glosses. It forms a deliberate mirror-pair with #63: this one, *"THE TRUTH IS A SPHERE,"* opens with the phrase written on the art itself, *ENLIGHTENED BEING;* the other, titled *"ENLIGHTENED BEING,"* opens with *THE TRUTH IS A SPHERE* — a phrase Roy spoke verbatim in a 2022 Listening Room Space. The title states the diagram's claim: truth is a sphere seen from many angles, and the enlightened being is the one whose awareness runs the whole radius, from body at the rim to Universal Self at the center.
**Connects to →** [[The Circle]] · [[Kabbalah - The Tree of Life]] · [[Roy]]

### #63 · ENLIGHTENED BEING · `THE TRUTH IS A SPHERE`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x2091eb048c499234810bd98c4384a5f941c21be16621cdffed180b4633e12b7f..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x2091eb048c499234810bd98c4384a5f941c21be16621cdffed180b4633e12b7f..jpg" width="300"></a>

A screenshot of Roy's own verified @JagwarTwin tweet, *"The truth is a sphere"* (5/4/21). It is the reciprocal of #61: that one is titled *THE TRUTH IS A SPHERE* and opens with *ENLIGHTENED BEING;* this one is titled *ENLIGHTENED BEING* and opens with *THE TRUTH IS A SPHERE,* so each is the other seen from the opposite side, literally enacting the aphorism that truth is a solid with no privileged face and every flat statement only a partial projection. The line sits in the Hermetic-perspectivist register this guide documents (the Kybalion's *"all truths are but half-truths"*) and had a second life as gameplay: Roy said *"truth is a sphere"* aloud in a 2022 Twitter Space and the community caught it in real time, proof that live events fed the maze.
**Connects to →** [[The Circle]] · [[Concepts/Source vs Reflection]] · [[Roy]]

### #69 · BLINKING (1/2) · `A565`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x18bec0c9ac882ddf3625770b4cb7c33324bddee295c713947635db9496c931ab..png"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x18bec0c9ac882ddf3625770b4cb7c33324bddee295c713947635db9496c931ab..png" width="300"></a>

The first half of the *"Blinking 🌞"* Notes draft (Nov 21, 2021), paired with #45. It opens with gnosis over belief (*"You don't have to believe anything / You just have to know / Know for yourself"*, knowledge held first-hand, not assented to), weighs the poles (*"Is it Left / Is it Right / is it Right to the Center?"*) and refuses the binary before the cosmology even starts: *"Both sides seem a little extreme / I think I'll take the Middle Way"* (the Buddhist middle path, and structurally the middle pillar between the Tree's two outer pillars). That couplet reads as the middle method spoken in verse, the operator #91 · *51.843* draws as geometry (the true middle holds *both* poles above them; the *"untrue balance"* is *"based on exclusion"*), and it is the doctrine of Roy's 2021 Discord post: *"The middle, the balance point always holds the truth, but it's essential to understand both sides."* From the chosen middle it then builds a nested cosmology: Moon → Sun → a Fiddle with Strings *"that plays every song"* → a Player → a Symphony Master who *"thinks of the Song."* The title echoes Roy’s tweet, *"We're almost literally in the eyeball of God as he/she/they are blinking,"* placing the whole draft inside the blinking eye of God (resonant with *El Roi,* *"the God who sees"*). A workshop for the *33* album's central myth: reality as a song played by ever-deeper unseen hands. Read as a pair, this half states the method and #45 performs it, running the regress behind the strings and landing it inward on *"It's just you inside yourself."*

Its opening matters to the interruption cluster: knowing for yourself is the faculty at stake when a program, performer, or metric takes over the sequence and tells the listener what comes next.
**Connects to →** [[The Circle]] · [[Concepts/Source vs Reflection]] · [[Concepts/The Voice]] · [[I Like to Party]] · [[The Interview]] · [[Kabbalah - The Tree of Life]] · [[El Roi]] · [[Roy]]

### #70 · THE CITY OF PYRAMIDS · `A-SITTING ON A GATE`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x1c511dc41bb38f13ee2bbaf0a7d9e8489a9a1c301c1d3bfe4d7961b4ffce9612..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x1c511dc41bb38f13ee2bbaf0a7d9e8489a9a1c301c1d3bfe4d7961b4ffce9612..jpg" width="300"></a>

This page can be read as an instruction in comparison. A song is drawn as a contour beside electrical polarity, while the self appears as both a 0–10 ladder and nested circles. *"Similar Structures"* is written over the Happy Birthday/current cluster, so it securely governs that local comparison; it does not make every mark on the sheet the same diagram. The ladder places *"Abyss (knowledge is relation)"* between 3 and 4 and ends *"10. The World"*. The concentric *Enlightened Being* counts the other way, from Self at the centre to body at the outside. Carrying a number from one figure to the other without checking direction will reverse it.

The sheet also contains *"Man devours God, God devours Man"*; the letting-go/hell note that points to room #131's code `LET GO`; and the numbered cross titled *"The Venus Connection"*, which becomes room #94. Four other rooms — Truth Is A Sphere, Roy HBD, Current-Sea, and Genesis Tree Code — isolate regions from this same source page. They remain separate doors, but together reveal that City is a hub rather than one more detached ladder.

The title has a real Thelemic/Crowley framework behind it: the City of the Pyramids is beyond the Abyss, and entry costs the aspirant's separate name and mastery, leaving dust in the city. That is a useful source-tradition, not proof Roy borrowed the title directly or a promise that form returns carrying experience. The written citation is `1 Kings 5:5`, where Solomon vows to build a house for the Name; *"in thy room"* means in David's place as successor, not in a Hall room. Finally, the code *"A-SITTING ON A GATE"* comes from Carroll's White Knight song and its name-versus-thing tangle. Here that literary problem becomes lock mechanics: a name can describe, misdirect, preserve, or open another door.
**Connects to →** [[Kabbalah - The Tree of Life]] · [[The Circle]] · [[Roy]]

### #76 · THE GATE OF THE WORLD · `D637`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x105696945071ed4aab05c06fe11d4825af2d0d3f2680ec6c52547263c1b50957..png"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x105696945071ed4aab05c06fe11d4825af2d0d3f2680ec6c52547263c1b50957..png" width="300"></a>

A pen sketch labeled *"Lemon the Rabbit (at the Gate of the World)":* a large-headed, stern-faced rabbit with a cross on its forehead and a stick-figure body, annotated *"Acorn grenade"* and *"Feet →,"* standing over a circled-cross ⊕ glyph — the classical astrological symbol for Earth, a fitting literal *"gate of the world"* beneath the rabbit's noted feet. The rabbit is Roy's recurring initiatory self-image (the Sir Rabbit family: Roy-as-rabbit in the *Good Time* dream, the Sir Rabbit poem, Fibonacci Rabbits), and *"the Gate of the World"* is the rabbit's own station: rung 10 of the animal ladder, the last stop of the count that descends from the source (swan 1, owl 2, on down), where emanation lands on Earth; in the tradition, "the Gate" is one of Malkuth's own titles. The drawing even performs the board's two-way arithmetic: the pointed-out *"Feet →"* are station 1 of the body count that climbs back up from the ground, so this one figure carries both ends of the ladder at once, 10 from above and 1 from below, the pair that makes eleven, standing on the Earth glyph at the gate where the descent turns into the return climb. Two more things on this page are load-bearing and read as throwaway. First the name: **Lemon** is one of only two domestic names in a bestiary otherwise populated by archangels and planets, the other being Cinnamon the Horse at Mars, and those two seats carry the ladder's two clearest explicit transformations of the speaker — *"the child **became** a warrior"* at five, *"I **woke** up in my shadow"* here at ten. (Other rungs record events, rung four's *"I found everything"* among them; what these two alone state is a subject changed into something.) Second the weapon: an **acorn grenade**. An acorn is a seed, a thing children fill their pockets with, the object an entire tree comes out of, and Roy labels it as ordnance. Among all ten bestiary figures, every face now read (the owl and the spider were the last two, 2026-09-01, and hold nothing), Lemon and Cinnamon are the only two shown holding recognizable weapons — a sweet name on an armed body, twice, at the two rungs where the cosmology touches a life someone actually lives. And the grenade has a striking counterpart in the songs: *Good Time (1:08)* — the song **where Roy is the rabbit**, born from the dream in which Sir Lucius first appears — sings *"Youth is not a fountain, **it's a hand grenade in hiding**."* The drawing is that line performed: youth's own object, an acorn, with the grenade hidden inside it, held by the maze's rabbit — and filed by the maze itself under **Unseen Things**, a grenade in hiding on the shelf of hidden things. The room was already in the maze by **August 2022**, more than a year before *Good Time* released in **September 2023**. The lyric's other half lives two shelves away: [[Artifacts - Gallery 2 (38-74)|#58 *A Storm in the Child's Fountain*]] is the same construction run on the same line's *fountain* — youth's fountain with the violence already inside it.
**Connects to →** [[Sir Rabbit]] · [[Roy]] · [[Sir Lucius]]

### #89 · SUPERFICIAL CONCLUSIONS · `SURFACE PERCEPTIONS`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x7240f1669388aaf35bc3f412f9b3d383a6c78f5a2d0d2c1504c814e18b6571e8..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x7240f1669388aaf35bc3f412f9b3d383a6c78f5a2d0d2c1504c814e18b6571e8..jpg" width="320" controls preload="none"></video>

A shaky night phone clip outside a concert venue: it opens on a blond figure in a khaki cap wearing an Enrique Iglesias & Pitbull co-headline tour tee, whip-pans across a dark parking lot, then lands on a crowd pressed against barricades under a stadium light as that same figure sprints toward them — the classic artist-exit *"appearance"* moment (the figure is not identified here, and the tour tee does not date the clip). The catalog's descriptor *"VIDEO APPEARANCE"* puns on both senses: making an appearance, and being only an appearance, and the clip cannot show whether the barricade crowd knows who is running at them. The code *SURFACE PERCEPTIONS* extends the warning: tidy readings of a person, like fan readings of a star, are conclusions drawn from surfaces. The clip invites a spectacle reading, then leaves us wondering how much we supplied ourselves.
**Connects to →** [[Concepts/The Circus#What it produces (the Spectacle)|the Spectacle]] · [[Concepts/The Audience]] · [[The Mask]] · [[Concepts/Communion vs Spectacle]] · [[Roy]]

### #93 · FORBIDDEN CITY · `ROS TAU`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xcd6b139806e198f303768272ee2839a71ea80daf58def1bc8f552451c49bec81..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xcd6b139806e198f303768272ee2839a71ea80daf58def1bc8f552451c49bec81..jpg" width="300"></a>

This still is a direct crop of the KHAWAH sheet, not a third independent Lambda drawing. The form is a later completed-Lambda reception of Plato's *Timaeus*: Plato supplies the seven boundary quantities and their harmonic means, but not this finished ten-node triangle or a full Lambdoma matrix.

A pencil sketch in the later completed-Lambda reception of Plato's *Timaeus:* *"Forbidden City"* titled at top, a tall triangle with 1 at the apex descending the doubling side (2, 4, 8) and the tripling side (3, 9, 27), with 6, 12, and 18 circled inside, captioned *"World Soul"* and *"Extended continuous geometric proportion."* The code *"ROS TAU"* names Rostau, the ancient Egyptian name for the Giza necropolis, the gateway to the Duat — so the sketch of hidden generative proportion sits under the name of Egypt's own forbidden precinct. The phrase *"Forbidden City"* also appears on Roy's board, placing this inside his documented esoteric notes: a diagram whose whole subject is generative proportion, the One unfolding into the many while remaining bound by ratio. The circled 6, 12, and 18 are the later diagram's interior means. They make visible the binding function Plato gives proportion — the fairest bond through which many terms become one body — without making the completed ten-node construction itself Plato's. The same figure recurs in room 48 and on the KHAWAH sheet of room 30, which sets the interior terms apart the same way and labels the figure with this room's name. The room is also the summit of a literal climb: room 92's code is room 91's name and this room's code is room 92's name, so the chain 91 → 92 → 93 is walked by typing each room's name into the next door, Higher Resolve, then the Great Pyramid's slope angle, then the Giza necropolis, then this mountain of numbers with 1 at the summit (the full circuit is drawn on [[Artifacts - The Wiring Diagram|the Wiring Diagram]]). That the climb ends at a city named as one you may not enter reads as the work's recurring threshold grammar: a summit approached, not crossed.
**Connects to →** [[Concepts/The Pattern]] · [[Roy]]

### #95 · SENSE HIERARCHY · `SENSE`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x3f0b71cd0d8353ced31b0012fa144e4df01029e80e53b219b72dd2d39211ea1d..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x3f0b71cd0d8353ced31b0012fa144e4df01029e80e53b219b72dd2d39211ea1d..jpg" width="300"></a>

A head in profile contains a banded pyramid, with *"1–10"* written above it and lines linking the structure to the facial region. This is a direct crop of the larger *GNOTHI SEAUTON* sheet, where the same head sits beside the freestanding Tree Thinking diagram. The two rooms keep separate names and codes, but the repeated image is one witness. In that context the bands read as an internal version of the ten-level structure, not a score from one to ten assigned to the senses. The lines show relation; their direction is not marked.

The code *"SENSE"* carries the picture from sensation toward understanding: receiving impressions is not yet making sense of them. Beside the City of Pyramids line *"knowledge is relation,"* the head becomes a model of perception organized into relationships. That sharpens the Upload's question, whether recording components preserves whoever experiences their relation. Surveillance is a possible misuse of this architecture, not the drawing's only or necessary subject.
**Connects to →** [[The Watchers]] · [[Pay Attention (feat. little luna)]] · [[Concepts/The Upload]] · [[Concepts/The Digital Double]] · [[Concepts/Witnessing vs Surveillance]] · [[Roy]]

### #98 · STARDUST · `VIBES`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xe9df1ddcba96373600cec1759f8c3c2a8c9cb15300a50ff8ed3744cf623b3f59..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xe9df1ddcba96373600cec1759f8c3c2a8c9cb15300a50ff8ed3744cf623b3f59..jpg" width="300"></a>

A black-and-white candid of two young shirtless men in board shorts laughing as they walk barefoot up wet sand on a tropical beach, a thatched beach hut and palms behind them; the slim blond figure is consistent with a young Roy of the Roy English era. Titling this ordinary happy moment *"STARDUST"* applies the maze's star-doctrine (*soul is a star,* every human being is a star) to two actual people, and the code *"VIBES"* matches the register exactly: good vibes, unguarded friendship, joy outside the machine. The setting is consistent with Bali, the one tropical destination Roy names in his catalog (*"Bali with the fam for the New Year,"* No Fake Friends), though unconfirmed.
**Connects to →** [[Soul Is A Star]] · [[Roy]]

### #99 · GATE OF THE WORLD · `J763`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xfa41edd355824bd4b4468112028214f508fc6e0cda6e6e2e113727ba1936151d..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xfa41edd355824bd4b4468112028214f508fc6e0cda6e6e2e113727ba1936151d..jpg" width="320" controls preload="none"></video>

A short silent turntable scan of a weathered grey garden rabbit statue, sitting on a captured tuft of real grass, rotating in a black void — a physical object uploaded into the digital maze. On Roy's board, stage 10 of the handwritten 0–10 emanation list is glossed *"The World"* (the Tree's final sephira), and the rabbit is the stage-10 animal of that ladder: Roy's searching-rabbit self at the station the ladder counts as 10 from above and 1 from below, the threshold where the inner climb meets the material World.
**Connects to →** [[Sir Rabbit]] · [[Kabbalah - The Tree of Life]] · [[Roy]]

### #102 · EDEN CUBE · `A RIVER`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x4c3cc16801611899e9e7698122fd86a5e82d6858095c4a4c47af5c46c51b91b9..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x4c3cc16801611899e9e7698122fd86a5e82d6858095c4a4c47af5c46c51b91b9..jpg" width="300"></a>

A pencil sketch: a small cube labeled *"EDEN"* hangs at the top of the page, and a river issues from it, widening as it descends into a large landmass; nested inside is a circle quartered by a cross, the classic world glyph. The code *"A RIVER"* reads as Genesis 2:10, *"a river went out of Eden to water the garden… and was parted into four heads,"* the quartered circle matching the four heads watering the four quarters of the world. Drawing Eden as a cube evokes the foursquare holy city of Revelation 21 and the Kabbalistic Cube of Space, and the whole image is a descent diagram — source flowing down into manifestation, the same top-to-bottom emanation the board runs to its final rung *"10. The World."* Notably it places Eden above and outside the world, the exact map the *33* lyric inverts by relocating Eden inside consciousness.
**Connects to →** [[Imagine A World]] · [[Kabbalah - The Tree of Life]] · [[Roy]]

### #110 · GATED · `THE GATE OF THE WORLD`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xd3521bdedaf866488bb7ca8d2bf6601b8e4104f3a405a90d139a460904d769a3..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xd3521bdedaf866488bb7ca8d2bf6601b8e4104f3a405a90d139a460904d769a3..jpg" width="320" controls preload="none"></video>

A 22-second night phone video looks through a tall spiked fence toward a crowd singing *Loser*, with brick apartment blocks behind them. The bars divide the image, but the song crosses the barrier. The title *GATED* makes that threshold part of the scene: separation and a shared voice in the same frame.
**Connects to →** [[Concepts/The Dweller on the Threshold]] · [[Roy]]

### #112 · SATURN MAGIC · `AN AGED AGED MAN`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x02a0cc9ce2413b24c6e24af980d6314fd74c072f232130a6a034fc5d1d3bd424..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x02a0cc9ce2413b24c6e24af980d6314fd74c072f232130a6a034fc5d1d3bd424..jpg" width="300"></a>

A pencil sheet places four related but historically distinct layers together: a played tic-tac-toe game; the Chinese Lo Shu magic square, `4-9-2 / 3-5-7 / 8-1-6`, whose rows sum to 15; the Renaissance planetary attribution of that square to Saturn; and the Seal of Saturn traced through it. The child's game and the talisman share a 3×3 field, so *"the game is to reveal the game"* is a useful project reading. It is not one ancient object with one origin.

The code *"AN AGED AGED MAN"* is a Carroll line from the White Knight's song in *Through the Looking-Glass*. Saturn/Kronos as aged Father Time makes a strong resonance, but the code does not identify the historical source of the diagram. This crop is also one region of the larger *How to Live Forever?* sheet, alongside Horus, Roy Strings, and God NFT.
**Connects to →** [[Concepts/The Pattern]] · [[Kabbalah - The Tree of Life]] · [[Roy]]

### #117 · ATTENTION'S GATE · `AHEAD OF STARDUST`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x15bfeefff0dd75eea8dcb5fdc19f75e3b7beab4e5664d418fecd2954722ca59d..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x15bfeefff0dd75eea8dcb5fdc19f75e3b7beab4e5664d418fecd2954722ca59d..jpg" width="300"></a>

A screenshot of a @JagwarTwin tweet (11/22/21), three aphoristic lines each sealed with an emoji: *"Ahead of stardust 🧠 / A sky carrying water ❄️ / Invisible world 👁"* (brain, snowflake, eye). The room title reframes the aphorism as a threshold: attention is the gate through which the invisible world is entered, the eye emoji its keyhole. The tweet condenses Roy's mind-before-matter cosmology — the brain sits *"ahead of"* stardust, i.e. consciousness precedes the star-stuff it inhabits — posted in the same weekend run as *"Words created everything we see, but everything we see began with a thought."* A compressed creation doctrine: mind, then medium, then the unseen world behind the seen. It belongs to the maze's Attention cluster alongside *Hello, Attention (Marduk).*
**Connects to →** [[The Veil — The Deep End]] · [[Concepts/Source vs Reflection]] · [[Pay Attention (feat. little luna)]] · [[Roy]]

### #118 · PREVIOUS FACE · `I AM THE INTERNET`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x349112c5088983866c49517cac2f743ff1248d45b5c6f5d62fce0591f8917b16..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x349112c5088983866c49517cac2f743ff1248d45b5c6f5d62fce0591f8917b16..jpg" width="320" controls preload="none"></video>

A short turntable scan floating on black: a glossy cobalt-blue sculpted human face, weathered and half-melted, an eroded hole at the cheek, affixed to an irregular slab of pale concrete with a small green sprig growing below it, the fragment slowly rotating like a museum relic. The title reads the object literally: a *"previous face"* is a cast-off former self preserved as archaeology, the discarded persona fossilized in concrete while new growth starts beside it (a Zen-koan resonance: *"your original face"*). The code *"I AM THE INTERNET"* is a seeded clue Roy tweeted from @JagwarTwin months earlier, the first-person completion of the board's title bar, *"Uploading my consciousness to the Internet."* Face and code together stage the Upload's bargain: the flesh face left behind as a relic, the self reborn as network.
**Connects to →** [[Concepts/The Upload]] · [[Concepts/The Algorithm]] · [[Concepts/The Digital Double]] · [[Roy]]

### #119 · HOPSCOTCH · `THE GAME IS TO REVEAL THE GAME`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xaf95179d663989bcaa6bb8542d4bd71866cc1690dcfd2115d5168a81324ab7e4..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xaf95179d663989bcaa6bb8542d4bd71866cc1690dcfd2115d5168a81324ab7e4..jpg" width="300"></a>

A clean screenshot of a verified @JagwarTwin tweet (11/19/21, 33-rollout era), reading *"The Game is to reveal the Game";* the code is the tweet's own text, so the room enacts its aphorism — the artifact reveals the game that unlocks it. The line is the closest thing the Hall of Mirrors has to a mission statement, framing the whole ARG (and Roy's attention-critique generally) as a game whose winning move is *exposure* of the game rather than escape from it. The title *"HOPSCOTCH"* casts that meta-game as a children's sidewalk game, hopping square to square (room to room) toward the end-square — the traditional court's Earth-to-Heaven ladder hovering behind it.
**Connects to →** [[Concepts/The Pattern]] · [[The Circle]] · [[Roy]]

### #120 · FIBONACCI RABBITS · `THE GRASSY KNOLLS`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x4c53b282a01a9469e203dd68bfc3d8d1a2de102960877395436052b23ceb5b12..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x4c53b282a01a9469e203dd68bfc3d8d1a2de102960877395436052b23ceb5b12..jpg" width="300"></a>

This is a direct crop of the KHAWAH sheet. The crop gets its own room and key, but it is not independent corroboration of the parent page's pattern argument.

A hand-drawn dendrogram of the classic *Liber Abaci* rabbit-breeding problem: pairs of rabbits branch downward through generations, the running totals 1, 1, 2, 3, 5, 8, 13 down the margin, captioned *"Rabbits in the key of 'F'"* — a pun collapsing F-for-Fibonacci into a musical key, mathematical growth *heard* as music. It is one of the board's sketches, filed there under the Hermetic Correspondence principle: *"The formula is the same from a solar system down to the atom… The Pattern is present on all levels."* The code *"THE GRASSY KNOLLS"* names the rabbits' habitat, with a winking plural of conspiracy-culture's *"grassy knoll."* The Pattern rendered at its most literal: one becomes two becomes many by a single repeating rule, nature's generative recursion drawn as a family tree.
**Connects to →** [[Concepts/The Pattern]] · [[Roy]]

### #121 · WHEELS · `THERE ARE THINGS BEHIND THINGS`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x53d191a03cbbb914ec3f6bb0e23d7b409c49e6990c897546f20b3cb6bef32d0a..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x53d191a03cbbb914ec3f6bb0e23d7b409c49e6990c897546f20b3cb6bef32d0a..jpg" width="300"></a>

A fibrous sphere is girdled by three rings labeled *"Ring Pass-Not (AIN SOPH AUR)", "Ring Chaos (AIN SOPH),"* and *"Ring Cosmos (AIN)"*, with an arrow rising at right. The diagram is a composite: Ain, Ain Soph, and Ain Soph Aur belong to the Hermetic-Qabalistic negative veils, while *Ring Pass-Not* is Theosophical language also used by William G. Gray. Naming those layers separately is more accurate than calling the whole page one ancient cosmology.

The code *"THERE ARE THINGS BEHIND THINGS"* states the room's operation cleanly: a visible boundary can conceal another register. The arrow may invite an ascent reading, but its destination is not labeled. This leaf is a direct crop of the larger *GNOTHI SEAUTON* sheet, alongside Tree Thinking and Ashim, so their shared marks are one source surface rather than three independent confirmations.
**Connects to →** [[Kabbalah - The Tree of Life]] · [[Concepts/The Pattern]] · [[Roy]]

### #122 · THE HOUSE CHANGES TO WATER · `V222`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xf6023f2878eb5f6028d3c879b9fe84215ed2df707c4603a22d92dd5e7e17790f..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xf6023f2878eb5f6028d3c879b9fe84215ed2df707c4603a22d92dd5e7e17790f..jpg" width="320" controls preload="none"></video>

A short looping scan of a real object rotating against black: a weathered grey barn-wood surface with a small circular photo pendant, encased in glass/resin and hung by string, showing a young couple in close embrace — a bespectacled man and a smiling woman in a personal, decades-old snapshot. It sits in the family/keepsake cluster (alongside *Grandma VI's Cow, Grandfather Masks,* *Matt and Roy*), so its weight is biographical rather than esoteric: a memento of kin and love-across-time hung on aged wood. The title *"THE HOUSE CHANGES TO WATER"* reframes that keepsake as a meditation on impermanence, the fixed structure of home and family dissolving into flux — an heirloom made navigable, memory literally turned into an object you can turn over and look behind.
**Connects to →** [[The Circle]] · [[Roy]]

### #123 · ANOTHER LIFE · `W1039`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xe6b2e437a9226d8878cc76d35b1801cdd7290890eabeff0dd5df35d4fea75095..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xe6b2e437a9226d8878cc76d35b1801cdd7290890eabeff0dd5df35d4fea75095..jpg" width="300"></a>

A heavily motion-blurred night-flash snapshot: a blond man (Roy) in a black hoodie with his arm around a young girl, both smiling, red cups and string lights streaking behind them in the dark. The extreme long-exposure smear turns the faces into ghost-trails, which is why the piece is filed *PHANTOMS* and titled *"ANOTHER LIFE"* — the blur literally renders the subjects as phantoms, and the title reads the ordinary human moment as a glimpse of a soul passing through one incarnation among many. Biographically a personal candid (Roy with what appears to be family), pulled into the maze as a *"phantom"* rather than a designed diagram or lyric card.
**Connects to →** [[Concepts/The Pattern]] · [[Roy]]

### #124 · ASHIM · `WHO AM I TO KNOW?`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xaeac4d9c7b5c0719e0c55cf6d9d3d5cf2862771d726f13e6ba39bd4207d84955..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xaeac4d9c7b5c0719e0c55cf6d9d3d5cf2862771d726f13e6ba39bd4207d84955..jpg" width="300"></a>

A handwritten note reads *"Ashim / Consciousness of an Atom"* above *"Degrees of Being alive."* The strongest title-and-text source is Alice A. Bailey's 1922 *The Consciousness of the Atom*, whose seventh lecture develops graded consciousness from atom upward. The exact study copy Roy used has not been identified, so Bailey is a strong source direction rather than a photographed-source receipt.

*Ashim* or *Ishim* also names an angelic class in Kabbalistic schemes, close to the human or material world. That tradition and Bailey's graded-consciousness argument sit beside each other here without becoming the same source. The code *"WHO AM I TO KNOW?"* keeps the page in its proper temperature: metaphysical speculation with its own doubt still attached. This is another direct crop of *GNOTHI SEAUTON*, not an independent page.
**Connects to →** [[Kabbalah - The Tree of Life]] · [[Concepts/The Pattern]] · [[Roy]]

### #125 · THE SERPENT MUST TAKE FLIGHT · `THE MONKEY MUST BECOME THE BIRD`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xfba584b226f83d7a734d58494875652f3179c776feab05907958323558285499..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xfba584b226f83d7a734d58494875652f3179c776feab05907958323558285499..jpg" width="300"></a>

A white line-drawn bald head over a mirror-doubled bed of roses (dark reds, oranges, purples). The face is serene and closed-eyed but bears an open third eye on the forehead and two protruding fangs at the mouth (the *"serpent"* / animal nature), and below the throat a symmetrical lotus/petal mandala reads as awakened lungs or a heart-chakra bloom. Both the title *"THE SERPENT MUST TAKE FLIGHT"* and the code *"THE MONKEY MUST BECOME THE BIRD"* encode the same idea — kundalini ascent and evolutionary self-transcendence: the coiled serpent (base energy) rising to the crown and opening the eye of vision, the *"monkey"* (restless lower self) transforming into the *"bird"* (flight, spirit, freedom). It dramatizes this guide's core human-to-superhuman tension: the drive to go beyond the merely human that runs through Roy's cosmology. And the formula has a picture. In the whole maze, monkey, serpent, and flight meet in exactly one other room: [[Artifacts - Gallery 1 (1-37)#16 · THE EMPTY ROOM|room 16, THE EMPTY ROOM]], where they are drawn rather than written, a two-faced monkey below trying to move both ways at once (the one that must become the bird), three serpents already in flight above, and between them the colorless rainbow bridge the crossing runs over. One room states the formula; the other pictures it.
**Connects to →** [[Concepts/The Upload]] · [[Superhuman]] · [[Kabbalah - The Tree of Life]] · [[Roy]]

### #132 · ROY'S LADDER · `C33`
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x4ccf3a893bc85a47b0f0f7fa643c87f2ff794f6afdccb15763373f3e76291373..mp4" poster="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x4ccf3a893bc85a47b0f0f7fa643c87f2ff794f6afdccb15763373f3e76291373..jpg" width="320" controls preload="none"></video>

A 3D scan of a literal stepladder: a paint-spattered aluminum household ladder with yellow feet, standing on a torn patch of scanned concrete and turning slowly against black. *(Corrected July 2026: this guide previously described this room as a framed family photograph whose easel back was the "ladder." That is the separate room [[Artifacts - Gallery 4 (113-149)#153 · FAMILY PORTRAIT|#153 · FAMILY PORTRAIT]] — the two were swapped, and the videos themselves settle it.)* The reading is better without the inference. Among the maze's diagrams and icons, Roy kept the most ordinary climbing object in the house and titled it *Roy's Ladder,* keyed *"C33"* to the *"I'm 33"* board and the album *33.* Every other ladder in this world is borrowed and exalted — the monastic *Ladder of Divine Ascent* pinned on the board, the *"How to Live Forever? Earth→Heaven"* hopscotch, the Tree's own rungs. This one is a thing you actually stand on to reach something. The ordinary made a step, with no metaphor required.
**Connects to →** [[Concepts/The Pattern]] · [[Kabbalah - The Tree of Life]] · [[Concepts/Jacob and Esau]] · [[Roy]]

### #146 · LOGOS (RATIO) · `ELEPHANT TORTISE EAGLE ATLAS`
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x764dca779c8de0d4e80a6508e91cf28577d0eac16dd7b0b2f0094d359d311ba5..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x764dca779c8de0d4e80a6508e91cf28577d0eac16dd7b0b2f0094d359d311ba5..jpg" width="300"></a>

A hand-drawn sheet is titled *"Logos (Ratio)"*, with a crossed axis and the list *"Elephant, Tortoise / Eagle, Atlas."* The code preserves Roy's spelling: `ELEPHANT TORTISE EAGLE ATLAS`. These are not four securely identified world-bearers. Roy's 2023 four-part riddle supplies the immediate route: Ganesha was an elephant; *tortoise* can be misspelled *tortise*; *"Matthew was an Eagle"*; and Atlas was Prometheus's brother with the world on his shoulder.

The answers are heterogeneous, and Roy's Matthew line should remain exactly as given. In standard Christian tetramorph iconography the eagle belongs to John, not Matthew, so the anomaly cannot be silently repaired into doctrine. *Logos* glossed as *ratio* still opens a real Greek double meaning, word/reason/proportion, while the crossed axis can invite an axis-mundi reading. Those are readings around the riddle, not its solved four-part category. The leaf is a direct crop of KHAWAH, which places it in a larger page about number, music, birth, and naming.
**Connects to →** [[Concepts/The Pattern]] · [[The Circle]] · [[Kabbalah - The Tree of Life]] · [[Roy]]

---

## Read next

- [[Artifacts]] — the hub, and the rest of the artifact groups.
- [[Hall of Mirrors]] — the ARG these objects were unlocked from.
- [[Hall of Mirrors/The I'm 33 Board]] — the cosmology board so many of these images are pieces of.
