---
title: Another Way II Heaven
type: song
cssclasses: ["type-song"]
status: published
confidence: mixed
source_basis: ["released music", "public interviews", "public posts", "official videos"]
---
# Another Way II Heaven

> **One-line reading.** A first-person creation myth: a solitary "I," unable to know itself alone, speaks itself into two, births a mirror, then a Darkness and a Light whose collision makes a Sun — multiplying toward eleven, reunion promised and escape quietly forbidden.

> [!info] Album & credits
> **Album:** *33* (2022, Big Loud Rock) — **track 8 of 10** · 4:25
> **Written by** Roy English (sole) · **Produced by** Roy English & Matt Pauling
> **Lyrics:** [Genius](https://genius.com) · streaming
> **▶ Listen:** [Spotify](https://open.spotify.com/track/3NN8oP3CLbYmQjizcGtu9s)

The creation myth *underneath* *33*'s attention-economy critique — one of the core mythology texts in the catalogue. Roy wrote this cosmogony **twice**: the lyric is near-identical to a standalone poem on his public "I'm 33" board, so the creation-myth reading isn't a listener's projection — it's the project's stated cosmology.

## The song is a count — One through Eleven

The song maps itself: it's a creation sequence told as a count, each number a stage of the One dividing in order to know itself. Following its own numbers:

| # | In the song | What it does |
|---|---|---|
| **1** | *"I was just me… but who was 'I' to know?"* | The solitary One. The problem is **sight** — it can't know itself alone (*"I wanted to see myself"*). |
| **2** | *"'One, become Two'… 'I' was in reverse… 'I' became 'You'"* | Division as self-knowledge. The **mirror is born**: "You" is "I" reversed. |
| **2 →** | *"a Darkness and a Light… a Sun was born inside"* | The first dyad collides and **ignites a Sun** — the union of opposites *makes* light. |
| **(3)** | *— skipped —* | Unnumbered. Possibly the Sun itself: the child of the union, not a division. |
| **4** | *"next there was 4, 'Process' and 'Time'"* · *"I opened up a Door"* | A structured world begins; a **door** opens and the self releases *"to cover the sky."* |
| **5–8** | *"split into Five, Six, Seven, Eight"* | Multiplication outward — the many proliferate. |
| **9** | *"when it came to Nine… I had to remind myself that 'I' was still 'I'"* | The point of **self-forgetting** — the one moment the self nearly loses itself in the many. |
| **10–11** | *"from 1:00 and through 11:00"* | The count is revealed as a **clock**; it runs to eleven and stops. |
| **(12)** | *— the missing hour —* | The unreached twelve loops back to One: the **deferred reunion**, *"one day we'll get back together and see The Great Design."* |

The refrain holds it all in place: *"I see it different every Time — another way to Heaven."* The song never arrives; it keeps **seeing differently.**

## What it means

A few claims that cut across the whole count:

- **The title *is* the argument.** *"To Heaven"* becomes *"II Heaven,"* the Roman two: the path *to* heaven is the path *of the two.* Multiplicity isn't the fall from unity — it's the route.
- **The founding dyad is the project's dyad.** *"A Darkness and a Light"* whose union makes a Sun is heart and mind, [[Roy]] and [[Sir Lucius]], placed at the very origin — and their meeting *generates* light rather than opposing the dark.
- **The captor's underside.** *"You'll never get away… don't you ever try to get away."* The One that divided itself also **forbids escape** — a closed system, not a free dispersal. This is where the cosmogony touches the [[Sir Lucius|Lucius]] closed-loop.
- **It's the charter for everything else.** Mirrors, doubles, twins, loops, fragmented selves, reunion — the whole JT vocabulary gets its **generating logic** here: unity divided itself to know itself, and every later song is a variation on that first act.

## How it connects

- **Characters:** [[Roy]] — the dividing "I" · [[Sir Lucius]] — the self-from-self division; the self-loss at "nine" points here.
- **Symbols:** [[The Mirror]] — born in this song · [[The Moon]]
- **Songs:** [[The Circle]] — the companion cosmology · [[Down to You]] · [[Soul Is A Star]] — the Sun/Son thread · [[Imagine A World]]

> [!note]- Deeper readings — the ninth station, the missing twelve, the Kabbalah *(tap to open)*
> **Nine as the "Lucius error."** In a Tree-of-Life frame the ninth station (Yesod) is where the self most risks mistaking the channel for the source. The lyric's lone moment of threatened self-loss landing exactly at nine is strikingly coherent with that. The song never says "Yesod"; the mapping is the reader's — but Kabbalah is demonstrably in Roy's toolkit (it's on the "I'm 33" board), so the *frame* is licensed even where the specific label is interpretive.
>
> **The missing twelve.** A clock has twelve; the count stops at eleven. The unreached twelve, which loops back to one, is the return — emanation runs 1→11, and 12→1 is deferred to *"The Great Design."* (One-through-eleven also reads as the ten Sefirot plus Da'at, the hidden eleventh.)
>
> **Emanation and return.** Read alongside Lurianic Kabbalah, the whole shape is procession-and-return: the One goes out into the many and is promised back.

## Lyrics

> [!quote]- Full lyrics<br><span class="jt-drawer-summary">The song's full released lyrics, kept folded.</span>
> ```
> Every every time
> Every every time (Every Time?)
> Every Time
> I see it different every Time
> Back long ago
> I was just me
> But who was "I" to Know?
> I wanted to see myself
> And where I could go
> (See it different every time)
> I spoke the words
> "One, become Two"
> Then "I" was in reverse
> But now "I" could see my Self
> And "I" became "You"
> Well,
> "You" and "I" could dance forever
> A Darkness and a Light
> (see it different every time)
> But,
> When we crashed and came together
> A Sun was born inside
> I see it different every time
> Another way to Heaven
> Another way to Heaven
> I see it different every Time
> From One and through Eleven
> Another way to Heaven
> Next there was 4
> "Process" and "Time"
> I opened up a Door
> And I just released myself
> To cover the sky
> (You'll never get away)
> Split into Five (5)
> Six
> Seven
> Eight
> But when it came to Nine...
> I had to remind myself
> That "I" was still "I"
> (I)
> Well,
> All of "Us" could dance forever
> In every Space and Time
> But,
> One day we'll get back together
> And see The Great Design
> I see it different every time
> Another way to Heaven
> Another way to Heaven
> I see it different every Time
> From 1:00 and through 11:00
> Another way II Heaven
> ♪
> See it different every time
> I see it different every time
> I see it different every time
> See it different every time
> Every Time
> Every Time
> I see it different every Time
> I see it different every Time
> Don't you ever try to get away
> Ya
> Don't you ever try to get away
> Ya
> ```

## Artifacts

**[[Artifacts - Gallery 1 (1-37)#18 · Tree Thinking|TREE THINKING]]** · `TREE THINKING` — The song is Roy's solo-authored emanation myth counting 'From One and through Eleven' (One become Two, splitting into 5,6,7,8,9, toward The Great Design); its essay reads the numbered procession as the ten S… → [[Artifacts - Drawings and Sketches|analysis]] · [gallery ↗](https://mirrors.jagwartwin.com/gallery)
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x8a9fe05bb3dbf271b4ab76f457b52a14dda44f3eb42edc5f4da3743febbe4bd9..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x8a9fe05bb3dbf271b4ab76f457b52a14dda44f3eb42edc5f4da3743febbe4bd9..jpg" width="200"></a>

**KHAWAH** · `26` — The song's lyric is documented (essay section 0) as near-identical to the cosmogonic poem on the same "I'm 33 this year" board this page belongs to, and the page's core equation, Adam minus Khawah = 26 = YHW… → [[Artifacts - Concept Images and Emblems|analysis]] · [gallery ↗](https://mirrors.jagwartwin.com/gallery)
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x9cce9eb03c9f29c6481fca9f0f942b15bef0bbbc47fda0ddb44df157019835d9..jpg"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x9cce9eb03c9f29c6481fca9f0f942b15bef0bbbc47fda0ddb44df157019835d9..jpg" width="200"></a>

**[[Artifacts - Gallery 2 (38-74)#43 · TRUE VOICE|TRUE VOICE]]** · `R1045` — 'True of voice' (maa-kheru) is the Egyptian judgment verdict that opens the deceased's way to the afterlife, and the song's held-identity gesture 'But when it came to Nine... I had to remind myself that I wa… → [[Artifacts - Ciphers and Code-Series|analysis]] · [gallery ↗](https://mirrors.jagwartwin.com/gallery)
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xf6b3674092ef945fda641263d2b24fa37e623df495c56c22448a423153316296..png"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xf6b3674092ef945fda641263d2b24fa37e623df495c56c22448a423153316296..png" width="200"></a>

**LAMBDOMA** · `REALITY IS THE CHILD OF ILLUSION` — Its cosmogony of "One, become Two" is the same descent the Lambda diagrams — the apex 1 dividing into the 2/3 legs and cascading to the many — this guide already links both to [[Concepts/The Pattern|The Pattern]] as the One-into-many… → [[Artifacts - Drawings and Sketches|analysis]] · [gallery ↗](https://mirrors.jagwartwin.com/gallery)
<a href="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xd722c486ba8a39b19cd81b4f5b5cac3b977934026d04ec6515e0f8d5ea6af4d9..png"><img src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0xd722c486ba8a39b19cd81b4f5b5cac3b977934026d04ec6515e0f8d5ea6af4d9..png" width="200"></a>

**ANOTHER WAY II HEAVEN** · `GENESIS TREE CODE` — This is the mint room for the released song itself (MINT BURIED, code GENESIS TREE CODE), and the code plus the dissolving wireframe head directly render the song's Genesis creation myth ('One, become Two')… → [[Artifacts - Songs, Demos and Mints|analysis]] · [gallery ↗](https://mirrors.jagwartwin.com/gallery)
<video src="https://storage.googleapis.com/jagwar-twin-33.appspot.com/artifacts/0x51f28bfcfda3b51406f62a37563c55c183d19edf4b4bc914535fb7f3f939932c..mp4" poster="https://plumcreekranch.com/jagwar/artifacts/139.png" width="240" controls preload="none"></video>
