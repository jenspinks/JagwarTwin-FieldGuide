...
/* Drawers (collapsible callouts) — keyboard access.
 *
 * Publish renders a callout's title as a plain <div> with a click handler, and a
 * folded callout's content as display:none. A keyboard user can neither reach the
 * toggle nor anything inside, so every drawer on the site is mouse- or touch-only.
 * This gives each foldable title button semantics: Tab reaches it, Enter or Space
 * toggles it through Publish's own click handler, and aria-expanded follows the
 * fold state. It changes no content and no appearance beyond the focus ring.
 */
(function () {
  "use strict";
  if (window.__jtCalloutKeys) return;
  window.__jtCalloutKeys = true;

  var SELECTOR = ".callout.is-collapsible > .callout-title:not([data-jt-keys])";
  ...
  function enhance(title) {
    // If Publish ever makes these focusable itself, stand down rather than double-toggle.
    if (title.hasAttribute("role") || title.tabIndex >= 0) { title.setAttribute("data-jt-keys", "native"); return; }
    title.setAttribute("data-jt-keys", "");
    title.setAttribute("role", "button");
    title.tabIndex = 0;
    sync(title);
    new MutationObserver(function () { sync(title); })
      .observe(title.parentElement, { attributes: true, attributeFilter: ["class"] });
    title.addEventListener("keydown", function (e) {
      if (e.target !== title || e.altKey || e.ctrlKey || e.metaKey) return;
  ...
  function scan() {
    ...
    for (var i = 0; i < titles.length; i++) {
      try { enhance(titles[i]); } catch (e) { /* never break the page */ }
    }
  }

  // Publish renders pages (and hover previews) on the fly; rescan shortly after any change.
  var pending = 0;
  function schedule() {
    if (pending) return;
  ...
  function start() {
    ...
    try { if (window.publish && window.publish.on) window.publish.on("navigated", schedule); } catch (e) { /* the observer covers it */ }
  }
